...
def empty_words(df):
  ...
  st.text(f"Выбери верное слово в предложении {sentences}")
  st.text(f'Варианты слов {variants}')
  clean_word=st.text_input()
  if clean_word==word:
    st.text('Поздравляем вы выбрали верное слово')
  else:
    st.text(f'Вы ошиблись, верное слово {word}')
